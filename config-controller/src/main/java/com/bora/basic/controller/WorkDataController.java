...
import com.bora.basic.dal.domain.WorkDataDo;
import com.bora.basic.service.service.IBasicDefineService;
import com.bora.basic.service.service.IWorkDataService;
 ...
 */
@RestController
@RequestMapping("/work/workDataDo")
...
public class WorkDataController {
    ...
    static{
        ...
        COLUMN_KEY.put("work_status","workStatus");
        COLUMN_KEY.put("device_code","deviceCode");
        COLUMN_KEY.put("com_code","comCode");
        COLUMN_KEY.put("failed_count","failedCount");
        COLUMN_KEY.put("open_time","openTime");
        COLUMN_KEY.put("finish_time","finishTime");
    ...
    public Result save(@RequestParam Map<String,Object> params){
        WorkDataDo workDataDo = ReflectUtil.mapToObject(params, COLUMN_KEY, WorkDataDo.class);
        if(workDataDo == null){
        ...
        //其他限制字段待定
        int tenantId = 1;
        workDataDo.setTenantId(tenantId);
        return Result.ok(iWorkDataService.save(workDataDo));
    }
    ...
    public Result edit(@RequestParam Map<String,Object> params){
        //为空直接返回，不做操作
        WorkDataDo workDataDo = ReflectUtil.mapToObject(params, COLUMN_KEY, WorkDataDo.class);
        if(workDataDo == null || workDataDo.getId() == null){
            return Result.error("您的操作有误！");
        }
        //其他限制字段待定
        return Result.ok(iWorkDataService.updateById(workDataDo));
    }
    ...
    public Result remove(@RequestParam("id") Long id){
        return Result.ok(iWorkDataService.removeById(id));
    }
    ...
    public Result getMaterialById(@RequestParam("id") Long id){
        WorkDataDo workDataDo = iWorkDataService.getById(id);
        if(workDataDo == null){
        ...
        Map<String, Object> objectMap = ReflectUtil.objectToMap(workDataDo);
    ...
    public Result listMaterialData(@RequestParam(value = "pageIndex", required = false, defaultValue = "1") Integer pageIndex,
        ...
        //分页查询
        QueryWrapper<WorkDataDo> queryWrapper = new QueryWrapper<>();
        queryWrapper.select(fields);
        IPage<Map<String, Object>> page = new Page<>(pageIndex, pageSize);
        IPage<Map<String, Object>> workDataDoIPage = iWorkDataService.pageMaps(page, queryWrapper);

        com.bora.commmon.page.Page page1 = new com.bora.commmon.page.Page(pageIndex, pageSize);
        page1.setTotal((int)workDataDoIPage.getTotal());
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("title",fieldNameMap);
        //对检索结果进行处理
        //拆分扩展字段合并到系统、可选字段同一等级
        List<Map<String, Object>> records = workDataDoIPage.getRecords();
